...
PRESSING:
  ...
  FLICKR
    XXX set up the incoming auth flow to catch the username
    XXX set up the api calls to be made properly
    XXX set up the feeds to pull through properly
    XXX deal with displaying
    >>> Get the date for the photos to organize the feed properly
