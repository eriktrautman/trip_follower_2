...
PRESSING:
> Port the docs over to evernote before deleting records of functionality!
...
> Admins automatically follow trips for which they are admins (but cannot unfollow them)
> I can view all my followed trips
...
> Unobtrusive Javascript: Hide a real form button if JS is enabled, so non JS users can use the site and I can write regular integration tests... Modernizr -- top HTML tags includes the browser's functionality as classes so you can use CSS to style based on availability of things like JS.
