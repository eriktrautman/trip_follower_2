...
ROADMAP:
	>> Thursday: Integrate Instagram, Research + Integrate Twitter, Research Tumblr (integrate?)
	>> Friday: User Feeds, Private logins for feeds
	>> Saturday: Finish private API login / authorization and user feeds
...
PRESSING:
XXX > Figure out sorting of media items
> Figure out best (basic) display of media items, eg. embedded cards
> Pull trip feeds into user feeds based on following (with refresh link if anything is AJAX'd)... or AJAX the feed in the first place?  Hmm.
Fri Mar 01 18:26:11 +0000 2013
  >> So set up the HTML wrappers to be different if current user versus a random user
  >> then in each respective wrapper load the correct JS code
  >> THEN set it up to work for Instagram and Tumblr
> Set up OAuth for private accounts
