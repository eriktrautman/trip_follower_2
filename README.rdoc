...
Parallax Slideshow!
> Create the blank html of the right size
> Populate it with a dozen or so divs
  >> Manage divs and their content with relative IDs (so div can be attached to its meta using equations)
  >> Set objects to respond to center coords only
  >> ugh.  coordinates.  Start at 0,0 then shift.
