...
Parallax Slideshow!
XXX Create the blank html of the right size
XXX Populate it with a dozen or so divs
  XXX Manage divs and their content with relative IDs (so div can be attached to its meta using equations)
  XXX Set objects to respond to center coords only
  XXX ugh.  coordinates.  Start at 0,0 then shift.
> XXX Set up the javascript modifiers for each div to work with the scrolling
> XXX fix the divs in the prev and next positions properly until their turn
> XXX resize and fade the divs properly until it's their turn
> XXX refactor the JS calculations to minimize CPU usage
> OOO Make the snap-to work
> XXX set up the navigation seeds
> XXX set up the meta trip content hoverbox
  >> XXX drop it down upon pageload
> XXX set up scroll prompt
> set up the item meta content to flash in and out
> populate the divs with content
> set up spacebar to jump properly
> check for window resize, modify configs so it still displays properly
> Webkit zoom perspective stuff
